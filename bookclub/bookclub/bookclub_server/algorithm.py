...
def match_algorithm(request):
    ...
    radius_lat_gt = user_lat + 0.81
    radius_long_gt = user_long + 2.1
    radius_lat_lt = user_lat - 0.81
    radius_long_lt = user_long - 2.1

    # get the users that are in the radius
    qs = User.objects.filter((Q(lat__gte=user_lat) & Q(long__gte=user_long) & Q(lat__lte=radius_lat_gt) & Q(long__lte=radius_long_gt)) |
    ...
    # add confirm trade into chatservice
